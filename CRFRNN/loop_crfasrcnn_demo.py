...
mypath = "/home/ubuntu/rcnn/images"
done = 0
doMaxFiles = 327
...
for f in myFiles:
    ...
    if ( not isfile(filename) ) or (not (".jpg" in filename)):
        continue
    IMAGE_FILE = filename
    print filename, done
    # caffe.set_mode_gpu()
    ...
    if maxDim > 500:
        scale = maxDim / 500.0
        # print scale,width/scale,height/scale
        image = resizeimage.resize_thumbnail(image, [int(width/scale), int(height/scale)])
        # image.save('test-image-cover.jpeg', 'jpeg')
    ...
    plt.savefig("/home/ubuntu/crfasrnn/segmented1/"+f)
    done += 1
print "DONE"
